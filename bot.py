...
async def get_ai_reading(prompt):
    try:
        completion = await groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "Ты — Наталья, опытный таролог. Стиль: мистический, краткий, четкий. Отвечай на русском."},
                {"role": "user", "content": prompt}
            ]
        )
        return completion.choices[0].message.content
    except Exception:
        return "Звезды сегодня туманны... Попробуйте чуть позже."
...
async def start_cmd(message: types.Message):
    await message.answer(
        ...
        "🤝 Если хочешь, чтобы я лично тебе помогла, выбери личный расклад, и напиши мне.",
